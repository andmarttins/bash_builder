...
describe('getApiRuntimeConfig', () => {
  it('rejects a non-HTTPS origin in production', () => {
    expect(() => getApiRuntimeConfig({
      NODE_ENV: 'production', API_PORT: '3000', APP_ORIGIN: 'http://app.example.com', DATABASE_URL: 'postgresql://a:b@db:5432/app', REDIS_URL: 'redis://:secret@redis:6379'
  ...
  it('requires an authenticated Redis URL with a supported protocol', () => {
    expect(() => getApiRuntimeConfig({
      NODE_ENV: 'production', API_PORT: '3000', APP_ORIGIN: 'https://app.example.com', DATABASE_URL: 'postgresql://a:b@db:5432/app', REDIS_URL: 'http://redis:6379'
