...
describe('PublicDashboardsController', () => {
  it('returns only the allowlisted static widget payload through the anonymous boundary', async () => {
    const tx = { dashboard: { findFirst: vi.fn().mockResolvedValue({ publicSnapshot: { title: 'Status operacional', description: 'Leitura pública', widgets: [
    ...
    expect(tx.dashboard.findFirst).toHaveBeenCalledWith({ where: { publicTokenHash: 'b'.repeat(64), published: true }, select: { publicSnapshot: true } });
  });
