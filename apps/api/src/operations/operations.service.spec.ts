...
describe('calculateHhtRates', () => {
  ...
  it('summarizes only the active tenant outbox health without reading event payloads', async () => {
    ...
    const tx = { outboxEvent: { groupBy: vi.fn().mockResolvedValue([{ status: 'PENDING', _count: { _all: 2 } }, { status: 'DEAD_LETTER', _count: { _all: 1 } }]), findFirst: vi.fn().mockResolvedValue({ availableAt: oldest }), count: vi.fn().mockResolvedValue(3) }, webhookDelivery: { groupBy: vi.fn().mockResolvedValue([{ status: 'DEAD_LETTER', _count: { _all: 2 } }]), count: vi.fn().mockResolvedValue(1) } };
    const tenants = { withTenantTransaction: vi.fn(async (_context, work) => work(tx)) };
    ...
    expect(summary.webhooks).toEqual({ pending: 0, processing: 0, delivered: 0, deadLetter: 2, cancelled: 0, expiredLeases: 1 });
  ...
  it('does not age a scheduled retry before it becomes actionable', async () => {
    const tx = { outboxEvent: { groupBy: vi.fn().mockResolvedValue([{ status: 'FAILED', _count: { _all: 1 } }]), findFirst: vi.fn().mockResolvedValue(null), count: vi.fn().mockResolvedValue(0) }, webhookDelivery: { groupBy: vi.fn().mockResolvedValue([]), count: vi.fn().mockResolvedValue(0) } };
