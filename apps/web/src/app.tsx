...
export function App(): React.JSX.Element {
  ...
  async function addFormField(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    setPending(true); setError(null);
    try {
      const form = (await api<{ form: FormSummary }>(`/v1/forms/${selectedForm.id}/fields`, { method: 'PUT', body: JSON.stringify({ expectedVersion: selectedForm.version, fields: [...selectedForm.fields, { key, label: values.get('label'), type, required: values.get('required') === 'on', options }] }) })).form;
