...
function HhtModulePage({ canManage }: { canManage: boolean }): React.JSX.Element {
  ...
  async function saveReport(event: FormEvent<HTMLFormElement>): Promise<void> { event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null); try { await api('/v1/hht/reports', { method: 'PUT', body: JSON.stringify({ companyId: values.get('companyId'), year: Number(values.get('year')), month: Number(values.get('month')), hhtWorked: Number(values.get('hhtWorked')), hhtMeal: Number(values.get('hhtMeal')), workforce: Number(values.get('workforce')), lostDays: Number(values.get('lostDays')), lti: Number(values.get('lti')) }) }); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível salvar o relatório.'); } finally { setPending(false); } }
