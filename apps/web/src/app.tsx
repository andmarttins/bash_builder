...
function ChangesModulePage({ canManage, canApprove, identity }: { canManage: boolean; canApprove: boolean; identity: Identity }): React.JSX.Element {
  ...
  const load = useCallback(async (): Promise<void> => {
    try { setChanges((await api<{ changes: Array<Record<string, unknown>> }>('/v1/changes')).changes); }
      ...
      {changes.length === 0 ? <p className="section-note">Nenhuma mudança registrada.</p> : <div className="form-list">{changes.map((change) => {
        ...
        return <article className="form-row event-detail" key={changeId ?? recordTitle(change)}>
          ...
          <div>
            ...
            {canManage && changeId && <form className="inline-form compact-form" onSubmit={(formEvent) => void addEvidence(changeId, formEvent)}><label>ID do arquivo pronto<input name="fileId" required pattern="[0-9a-fA-F-]{36}" title="UUID do arquivo listado no módulo Arquivos" /></label><label>Categoria<input name="category" maxLength={80} placeholder="Ex.: Plano de retorno" /></label><label>Descrição<input name="description" maxLength={10000} /></label><button className="secondary-button compact" type="submit" disabled={pending}>Vincular evidência</button></form>}
