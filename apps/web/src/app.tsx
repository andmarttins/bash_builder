...
function OperationsHealthPage({ canManage }: { canManage: boolean }): React.JSX.Element {
  ...
  const [webhookDeadLetters, setWebhookDeadLetters] = useState<Array<Record<string, unknown>>>([]);
  const [auditEntries, setAuditEntries] = useState<Array<Record<string, unknown>>>([]);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState(false);
  const load = useCallback(async (): Promise<void> => {
    try {
      const [summaryResponse, deadLetterResponse, webhookDeadLetterResponse, auditResponse] = await Promise.all([
        api<Record<string, unknown>>("/v1/operations/summary"),
        api<{ events: Array<Record<string, unknown>> }>("/v1/operations/outbox/dead-letter"),
        api<{ deliveries: Array<Record<string, unknown>> }>("/v1/operations/webhooks/dead-letter"),
        api<{ items: Array<Record<string, unknown>> }>("/v1/operations/audit"),
      ]);
      setSummary(summaryResponse);
      setDeadLetters(deadLetterResponse.events);
      setWebhookDeadLetters(webhookDeadLetterResponse.deliveries);
  ...
  const webhooks = isRecord(summary?.webhooks) ? summary.webhooks : undefined;
  ...
  async function redriveWebhook(deliveryId: string): Promise<void> {
    if (!window.confirm("Reenfileirar esta entrega após confirmar que o destino está ativo e a causa foi corrigida?")) return;
    setPending(true); setError(null);
    try { await api(`/v1/operations/webhooks/dead-letter/${deliveryId}/redrive`, { method: "POST" }); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Não foi possível reenfileirar a entrega de webhook."); }
    finally { setPending(false); }
  }
  return <><p className="eyebrow">Operação segura</p><h1 id="dashboard-title">Saúde operacional</h1><p className="description">Acompanhe filas, entregas e ações recentes da sua organização antes do piloto. Dados de outras empresas, payloads, segredos e erros de provedores não são expostos.</p>{error && <p className="form-error" role="alert">{error}</p>}{isRecord(outbox) && <><section className="overview-grid"><article className="overview-card"><span>Estado SLI</span><strong aria-live="polite">{status}</strong><small>Meta: itens acionáveis em até {String(sli?.targetMaximumAgeSeconds ?? 300)} s.</small></article><article className="overview-card"><span>Idade acionável</span><strong>{age}s</strong><small>Crítico a partir de {String(sli?.criticalMaximumAgeSeconds ?? 900)} s.</small></article><article className="overview-card"><span>Pendentes</span><strong>{String(outbox.pending ?? 0)}</strong></article><article className="overview-card"><span>Processando</span><strong>{String(outbox.processing ?? 0)}</strong></article><article className="overview-card"><span>Falhas</span><strong>{String(outbox.failed ?? 0)}</strong></article><article className="overview-card"><span>DLQ de eventos</span><strong>{String(outbox.deadLetter ?? 0)}</strong></article><article className="overview-card"><span>Leases vencidos</span><strong>{String(outbox.expiredLeases ?? 0)}</strong></article></section>{isRecord(webhooks) && <section className="admin-section"><h2>Entregas de webhook</h2><div className="overview-grid"><article className="overview-card"><span>Pendentes</span><strong>{String(webhooks.pending ?? 0)}</strong></article><article className="overview-card"><span>Processando</span><strong>{String(webhooks.processing ?? 0)}</strong></article><article className="overview-card"><span>Entregues</span><strong>{String(webhooks.delivered ?? 0)}</strong></article><article className="overview-card"><span>DLQ</span><strong>{String(webhooks.deadLetter ?? 0)}</strong></article><article className="overview-card"><span>Canceladas</span><strong>{String(webhooks.cancelled ?? 0)}</strong><small>Destino desativado ou reconfigurado.</small></article><article className="overview-card"><span>Leases vencidos</span><strong>{String(webhooks.expiredLeases ?? 0)}</strong></article></div></section>}{deadLetters.length > 0 && <section className="admin-section"><h2>Eventos em DLQ</h2><p className="section-note">Corrija a causa antes de reenfileirar. Payloads não são exibidos. A lista mostra até 100 itens.</p><ul className="nested-list">{deadLetters.map((event) => { const eventId = stringValue(event.id); return <li key={eventId ?? recordTitle(event)}>{stringValue(event.eventType) ?? "Evento"} · tentativa {String(event.attemptCount ?? 0)}{canManage && eventId && <button className="secondary-button compact" type="button" disabled={pending} onClick={() => void redrive(eventId)}>Reenfileirar</button>}</li>; })}</ul></section>}{webhookDeadLetters.length > 0 && <section className="admin-section"><h2>Webhooks em DLQ</h2><p className="section-note">A entrega só será reenviada se o destino e a referência de segredo armazenados ainda corresponderem à integração ativa. A lista mostra até 100 itens.</p><ul className="nested-list">{webhookDeadLetters.map((delivery) => { const deliveryId = stringValue(delivery.id); return <li key={deliveryId ?? recordTitle(delivery)}>{stringValue(delivery.eventType) ?? "Evento"} · tentativa {String(delivery.attemptCount ?? 0)}{canManage && deliveryId && <button className="secondary-button compact" type="button" disabled={pending} onClick={() => void redriveWebhook(deliveryId)}>Reenfileirar</button>}</li>; })}</ul></section>}<section className="admin-section"><h2>Atividade auditável recente</h2>{auditEntries.length === 0 ? <p className="section-note">Nenhuma atividade registrada.</p> : <ul className="nested-list">{auditEntries.map((entry) => <li key={stringValue(entry.id) ?? recordTitle(entry)}>{stringValue(entry.action) ?? "Ação"} · {stringValue(entry.resourceType) ?? "recurso"} · {stringValue(entry.occurredAt) ? new Date(stringValue(entry.occurredAt)!).toLocaleString("pt-BR") : "horário indisponível"}</li>)}</ul>}</section></>}<button className="secondary-button compact" type="button" onClick={() => void load()} disabled={pending}>Atualizar</button></>;
}
