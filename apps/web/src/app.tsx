...
function ChangesModulePage({ canManage }: { canManage: boolean }): React.JSX.Element {
  ...
  return <><p className="eyebrow">Módulo empresarial</p><h1 id="dashboard-title">Gestão de mudanças</h1><p className="description">Registre riscos antes da aprovação e conduza a solicitação pelas etapas controladas do processo.</p>{error && <p className="form-error" role="alert">{error}</p>}{canManage && <form className="inline-form admin-section" onSubmit={create}><label>Código<input name="publicCode" required minLength={2} maxLength={32} placeholder="MUD-2026-001" /></label><label>Título<input name="title" required minLength={2} maxLength={200} /></label><label>Solicitante<input name="requestedBy" maxLength={160} /></label><label>Responsável<input name="owner" maxLength={160} /></label><label>Prazo<input name="dueAt" type="datetime-local" /></label><label>Descrição<textarea name="description" maxLength={10000} /></label><button className="primary-button compact" type="submit" disabled={pending}>Criar mudança</button></form>}<section className="admin-section"><h2>Solicitações da organização</h2>{changes.length === 0 ? <p className="section-note">Nenhuma mudança registrada.</p> : <div className="form-list">{changes.map((change) => { const changeId = stringValue(change.id); const version = typeof change.version === 'number' ? change.version : 0; const risks = Array.isArray(change.risks) ? change.risks.filter(isRecord) : []; return <article className="form-row event-detail" key={changeId ?? recordTitle(change)}><Settings2 aria-hidden="true" /><div><strong>{stringValue(change.publicCode) ?? 'MUD'} · {recordTitle(change)}</strong><small>{stringValue(change.status) ?? 'DRAFT'} · etapa {String(change.currentStep ?? 1)} · {risks.length} risco(s)</small>{risks.length > 0 && <ul className="nested-list">{risks.map((risk) => <li key={stringValue(risk.id) ?? recordTitle(risk)}>{stringValue(risk.hazard) ?? 'Risco'} · P{String(risk.probability)} × S{String(risk.severity)} = {String(risk.score ?? Number(risk.probability) * Number(risk.severity))}</li>)}</ul>}{canManage && changeId && <><form className="inline-form compact-form" onSubmit={(formEvent) => void addRisk(changeId, formEvent)}><label>Perigo<input name="hazard" required minLength={2} maxLength={300} /></label><label>Probabilidade<select name="probability" defaultValue="3">{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Severidade<select name="severity" defaultValue="3">{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Controles<textarea name="controls" maxLength={10000} /></label><button className="secondary-button compact" type="submit" disabled={pending}>Adicionar risco</button></form><div className="action-row"><select aria-label="Nova etapa da mudança" defaultValue=""><option value="" disabled>Alterar etapa</option>{['DRAFT', 'IN_REVIEW', 'APPROVED', 'IMPLEMENTING', 'COMPLETED', 'REJECTED'].map((status) => <option key={status} value={status}>{status}</option>)}</select><button className="secondary-button compact" type="button" disabled={pending || version < 1} onClick={(buttonEvent) => { const status = (buttonEvent.currentTarget.previousElementSibling as HTMLSelectElement | null)?.value; if (status) void transition(changeId, version, status); }}>Atualizar</button></div></>}</div></article>; })}</div>}</section></>;
}
