...
type FormSubmission = { id: string; status: 'RECEIVED' | 'IN_REVIEW' | 'RESOLVED' | 'REJECTED'; submittedAt: string; formVersion: number; formSnapshot: { fields?: Array<{ key: string; label: string }> }; answers: Record<string, unknown> };
...
export function App(): React.JSX.Element {
  ...
  async function openForm(formId: string): Promise<void> {
    ...
    try {
      ...
      if (identity && (identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN' || identity.membership.role === 'MEMBER')) await loadSubmissions(formId, 1, submissionStatusFilter);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível abrir o formulário.'); }
    finally { setPending(false); }
  }
  async function loadSubmissions(formId: string, requestedPage = 1, status = submissionStatusFilter): Promise<void> {
    const { page, cursor } = resolveSubmissionCursorPage({ requestedPage, currentPage: submissionPagination.page, nextCursor: submissionPagination.nextCursor, pageCursors: submissionCursorHistory });
    const query = new URLSearchParams({ pageSize: '25' });
    if (status) query.set('status', status);
    if (cursor) query.set('cursor', cursor);
    const response = await api<{ submissions: FormSubmission[]; pagination: SubmissionPagination }>(`/v1/forms/${formId}/submissions?${query.toString()}`);
    setSubmissions(response.submissions); setSubmissionPagination({ ...response.pagination, page }); setSubmissionStatusFilter(status);
    setSubmissionCursorHistory((history) => page <= 1 ? [null] : [...history.slice(0, page - 1), cursor]);
  }
  async function updateSubmissionStatus(submission: FormSubmission, status: FormSubmission['status']): Promise<void> {
    if (!selectedForm || status === submission.status) return;
    setPending(true); setError(null);
    try {
      await api(`/v1/forms/${selectedForm.id}/submissions/${submission.id}`, { method: 'PATCH', body: JSON.stringify({ expectedStatus: submission.status, status }) });
      await loadSubmissions(selectedForm.id, submissionPagination.page);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível atualizar a tratativa.'); }
    finally { setPending(false); }
  }
  ...
  async function addFormField(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const values = new FormData(event.currentTarget); const key = slugValue(String(values.get('label') ?? '')); const type = String(values.get('type')) as FormFieldSummary['type']; const options = ['SELECT', 'MULTI_SELECT'].includes(type) ? String(values.get('options') ?? '').split(',').map((item) => item.trim()).filter(Boolean) : [];
    if (['SELECT', 'MULTI_SELECT'].includes(type) && options.length === 0) { setError('Informe ao menos uma opção, separada por vírgula.'); return; }
    setPending(true); setError(null);
    try {
      await replaceFormFields([...selectedForm.fields, { key, label: String(values.get('label')), type, required: values.get('required') === 'on', options, position: selectedForm.fields.length }]); event.currentTarget.reset();
  ...
  async function replaceFormFields(fields: FormFieldSummary[]): Promise<void> {
    ...
    const normalized = fields.map((field, position) => ({ ...field, position }));
    const form = (await api<{ form: FormSummary }>(`/v1/forms/${selectedForm.id}/fields`, { method: 'PUT', body: JSON.stringify({ expectedVersion: selectedForm.version, fields: normalized }) })).form;
    setSelectedForm(form); await refreshForms(form.id);
  }
  async function removeFormField(key: string): Promise<void> {
    if (!selectedForm) return; setPending(true); setError(null);
    try { await replaceFormFields(selectedForm.fields.filter((field) => field.key !== key)); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível remover o campo.'); }
    finally { setPending(false); }
  }
  async function moveFormField(key: string, direction: -1 | 1): Promise<void> {
    if (!selectedForm) return; const index = selectedForm.fields.findIndex((field) => field.key === key); const target = index + direction;
    if (index < 0 || target < 0 || target >= selectedForm.fields.length) return;
    const fields = [...selectedForm.fields]; [fields[index], fields[target]] = [fields[target]!, fields[index]!]; setPending(true); setError(null);
    try { await replaceFormFields(fields); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível reordenar os campos.'); }
    finally { setPending(false); }
  }
  async function updateFormMetadata(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); if (!selectedForm) return; const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { const form = (await api<{ form: FormSummary }>(`/v1/forms/${selectedForm.id}`, { method: 'PATCH', body: JSON.stringify({ expectedVersion: selectedForm.version, title: values.get('title'), description: values.get('description') || null }) })).form; setSelectedForm(form); await refreshForms(form.id); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível atualizar o formulário.'); }
    finally { setPending(false); }
  }
  async function copyPublicLink(): Promise<void> {
    if (!selectedForm) return;
    try { await navigator.clipboard.writeText(`${window.location.origin}/f/${selectedForm.publicId}`); setError(null); }
    catch { setError('Não foi possível copiar automaticamente; copie o link exibido.'); }
  }
  async function publishForm(expiresAt?: string): Promise<void> {
    if (!selectedForm) return; setPending(true); setError(null);
    try { const form = (await api<{ form: FormSummary }>(`/v1/forms/${selectedForm.id}/publication`, { method: 'POST', body: JSON.stringify({ expectedVersion: selectedForm.version, expiresAt: expiresAt ? new Date(expiresAt).toISOString() : null }) })).form; setSelectedForm(form); await refreshForms(form.id); }
  ...
  if (mode === 'signed-in' && identity) return <DashboardShell collapsed={sidebarCollapsed} identity={identity} onLogout={() => void logout()} onNavigate={setWorkspaceView} onToggle={() => setSidebarCollapsed((collapsed) => !collapsed)} pending={pending} unreadNotifications={unreadNotifications} view={workspaceView}><section className="panel dashboard dashboard-wide" aria-labelledby="dashboard-title">
    ...
    {workspaceView === 'home' ? <>
      ...
      {selectedForm && <section className="admin-section" aria-labelledby="form-editor-title"><h2 id="form-editor-title">{selectedForm.title}</h2><p className="section-note">Versão {selectedForm.version} · {selectedForm.status}{selectedForm.status === 'PUBLISHED' && <> · Link: <code>{window.location.origin}/f/{selectedForm.publicId}</code></>}</p>{(identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN') && <form className="inline-form" onSubmit={updateFormMetadata}><label>Título<input name="title" required minLength={2} maxLength={160} defaultValue={selectedForm.title} /></label><label>Descrição<input name="description" maxLength={10000} defaultValue={selectedForm.description ?? ''} /></label><button className="secondary-button compact" type="submit" disabled={pending}>Salvar dados</button></form>}<div className="form-list">{selectedForm.fields.map((field, index) => <article className="form-row" key={field.key}><FileText aria-hidden="true" /><div><strong>{field.label}</strong><small>{field.type} · {field.required ? 'obrigatório' : 'opcional'}{field.options.length > 0 ? ` · ${field.options.join(', ')}` : ''}</small></div>{(identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN') && <span className="field-actions"><button className="secondary-button compact" type="button" onClick={() => void moveFormField(field.key, -1)} disabled={pending || index === 0}>↑</button><button className="secondary-button compact" type="button" onClick={() => void moveFormField(field.key, 1)} disabled={pending || index === selectedForm.fields.length - 1}>↓</button><button className="secondary-button compact" type="button" onClick={() => void removeFormField(field.key)} disabled={pending}>Remover</button></span>}</article>)}</div>{(identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN') && <><form className="inline-form" onSubmit={addFormField}><label>Novo campo<input name="label" required minLength={2} maxLength={160} /></label><label>Tipo<select name="type" defaultValue="SHORT_TEXT"><option value="SHORT_TEXT">Texto curto</option><option value="LONG_TEXT">Texto longo</option><option value="NUMBER">Número</option><option value="DATE">Data</option><option value="SELECT">Seleção única</option><option value="MULTI_SELECT">Seleção múltipla</option><option value="CHECKBOX">Confirmação</option></select></label><label>Opções<input name="options" maxLength={1000} placeholder="Uma, duas, três" /></label><label className="checkbox-label"><input name="required" type="checkbox" /> Obrigatório</label><button className="secondary-button compact" type="submit" disabled={pending}>Adicionar campo</button></form><div className="action-row">{selectedForm.status === 'PUBLISHED' ? <><button className="secondary-button compact" type="button" onClick={() => void copyPublicLink()} disabled={pending}>Copiar link</button><button className="secondary-button compact" type="button" onClick={() => void revokeForm()} disabled={pending}>Revogar link público</button></> : <form className="inline-form" onSubmit={(event) => { event.preventDefault(); void publishForm(String(new FormData(event.currentTarget).get('expiresAt') ?? '')); }}><label>Expira em (opcional)<input name="expiresAt" type="datetime-local" /></label><button className="primary-button compact" type="submit" disabled={pending || selectedForm.fields.length === 0}>Publicar e gerar novo link</button></form>}</div></>}<h3>Respostas</h3><form className="inline-form compact-form" onSubmit={(event) => { event.preventDefault(); void loadSubmissions(selectedForm.id, 1, submissionStatusFilter); }}><label>Status<select aria-label="Filtrar respostas por status" value={submissionStatusFilter} onChange={(event) => setSubmissionStatusFilter(event.currentTarget.value as FormSubmission['status'] | '')}><option value="">Todas</option><option value="RECEIVED">Recebidas</option><option value="IN_REVIEW">Em análise</option><option value="RESOLVED">Resolvidas</option><option value="REJECTED">Rejeitadas</option></select></label><button className="secondary-button compact" type="submit" disabled={pending}>Filtrar</button><small>{submissionPagination.total} resposta(s)</small></form>{submissions.length === 0 ? <p className="section-note">Ainda não há respostas para este filtro.</p> : <div className="form-list">{submissions.map((submission) => <article className="form-row event-detail" key={submission.id}><ClipboardList aria-hidden="true" /><div><strong>{submission.status}</strong><small>Versão {submission.formVersion} · {new Date(submission.submittedAt).toLocaleString('pt-BR')}</small><dl className="identity-card">{(submission.formSnapshot.fields ?? []).map((field) => <div key={field.key}><dt>{field.label}</dt><dd>{formatSubmissionAnswer(submission.answers[field.key])}</dd></div>)}</dl>{(identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN') && <div className="action-row"><select aria-label={`Novo status da resposta ${submission.id}`} defaultValue=""><option value="" disabled>Tratar resposta</option>{submission.status === 'RECEIVED' && <><option value="IN_REVIEW">Iniciar análise</option><option value="REJECTED">Rejeitar</option></>}{submission.status === 'IN_REVIEW' && <><option value="RESOLVED">Resolver</option><option value="REJECTED">Rejeitar</option></>}</select><button className="secondary-button compact" type="button" disabled={pending || submission.status === 'RESOLVED' || submission.status === 'REJECTED'} onClick={(event) => { const status = (event.currentTarget.previousElementSibling as HTMLSelectElement | null)?.value as FormSubmission['status'] | undefined; if (status) void updateSubmissionStatus(submission, status); }}>Salvar tratativa</button></div>}</div></article>)}</div>}{submissionPagination.total > submissionPagination.pageSize && <div className="action-row"><button className="secondary-button compact" type="button" disabled={pending || submissionPagination.page === 1} onClick={() => void loadSubmissions(selectedForm.id, submissionPagination.page - 1)}>Anterior</button><span>Página {submissionPagination.page}</span><button className="secondary-button compact" type="button" disabled={pending || submissionPagination.page * submissionPagination.pageSize >= submissionPagination.total} onClick={() => void loadSubmissions(selectedForm.id, submissionPagination.page + 1)}>Próxima</button></div>}</section>}
    ...
    </> : workspaceView === 'events' ? <EventsModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : workspaceView === 'changes' ? <ChangesModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} canApprove={identity.membership.role !== 'VIEWER'} identity={identity} /> : workspaceView === 'bash' ? <BashModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : workspaceView === 'hht' ? <HhtModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : workspaceView === 'dashboards' ? <DashboardsModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : workspaceView === 'tv' ? <TvModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : workspaceView === 'integrations' ? <IntegrationsModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : workspaceView === 'files' ? <FilesModulePage canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} /> : <OperationalModulePage view={workspaceView} canManage={identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN'} />}
...
type OperationalView = Exclude<WorkspaceView, 'home' | 'forms' | 'notifications' | 'dashboards' | 'tv' | 'integrations' | 'files' | 'organization' | 'profile'>;
...
function OperationalModulePage({ view, canManage }: { view: OperationalView; canManage: boolean }): React.JSX.Element {
  ...
  return <>
    ...
    {error && <p className="form-error" role="alert">{error}</p>}
    {canManage && <form className="inline-form admin-section" onSubmit={create}><label>Título ou nome<input name="title" required minLength={2} maxLength={200} placeholder={page.manageLabel} /></label><button className="primary-button compact" type="submit" disabled={pending}>{pending ? 'Salvando…' : page.manageLabel}</button></form>}
...
function FilesModulePage({ canManage }: { canManage: boolean }): React.JSX.Element {
  const [files, setFiles] = useState<Array<Record<string, unknown>>>([]);
  const [upload, setUpload] = useState<{ supported: boolean; maxByteSize: number; contentTypes: string[] } | null>(null);
  const [notice, setNotice] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState(false);
  const load = useCallback(async (): Promise<void> => {
    try {
      const [fileResponse, configuration] = await Promise.all([api<{ files: Array<Record<string, unknown>> }>('/v1/files'), api<{ upload: { supported: boolean; maxByteSize: number; contentTypes: string[] } }>('/v1/files/configuration')]);
      setFiles(fileResponse.files); setUpload(configuration.upload);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível carregar os arquivos.'); }
  }, []);
  useEffect(() => { void load(); }, [load]);
  ...
  async function uploadFile(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!(file instanceof File) || file.size === 0) { setError('Selecione um arquivo válido.'); return; }
    if (!upload?.supported) { setError('O armazenamento privado ainda não está configurado.'); return; }
    if (!upload.contentTypes.includes(file.type) || file.size > upload.maxByteSize) { setError('O arquivo não atende ao tipo ou tamanho permitido.'); return; }
    setPending(true); setError(null); setNotice(null);
    try {
      const bytes = await file.arrayBuffer(); const checksum = await sha256Hex(bytes);
      const intent = await api<{ asset: { id: string }; upload: { supported: boolean; url: string } }>('/v1/files/intents', { method: 'POST', body: JSON.stringify({ originalName: file.name, contentType: file.type, byteSize: file.size, checksum }) });
      if (!intent.upload.supported) throw new Error('O armazenamento privado não está disponível.');
      await api(`/v1/files/${intent.asset.id}/content`, { method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: bytes });
      event.currentTarget.reset(); setNotice('Arquivo enviado e validado. Ele já pode ser vinculado como evidência em uma mudança.'); await load();
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível enviar o arquivo.'); }
    finally { setPending(false); }
  }

  async function cancelUpload(fileId: string): Promise<void> {
    setPending(true); setError(null); setNotice(null);
    try { await api(`/v1/files/${fileId}/cancel`, { method: 'POST' }); setNotice('Upload pendente cancelado com segurança.'); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível cancelar o upload.'); }
    finally { setPending(false); }
  }

  return <>
    <p className="eyebrow">Módulo empresarial</p><h1 id="dashboard-title">Arquivos</h1><p className="description">O upload é autenticado, passa pela API e fica privado no armazenamento S3 compatível. Links públicos não são gerados.</p>
    {error && <p className="form-error" role="alert">{error}</p>}{notice && <p className="section-note" role="status">{notice}</p>}
    {upload && !upload.supported && <section className="admin-section"><h2>Armazenamento pendente</h2><p className="section-note">Configure o armazenamento privado no Dokploy. O scanner interno também precisa estar saudável. Nenhuma intenção de upload será criada até que a cadeia esteja disponível.</p></section>}
    {canManage && <form className="inline-form admin-section" onSubmit={uploadFile}><label>Arquivo<input name="file" type="file" required accept=".pdf,image/jpeg,image/png,.xlsx,.docx" disabled={!upload?.supported || pending} /></label><p className="section-note">PDF, JPG, PNG, XLSX ou DOCX · até {upload ? `${Math.floor(upload.maxByteSize / (1024 * 1024))} MB` : '…'}.</p><button className="primary-button compact" type="submit" disabled={!upload?.supported || pending}>{pending ? 'Enviando…' : 'Enviar arquivo'}</button></form>}
    <section className="admin-section"><h2>Arquivos da organização</h2>{files.length === 0 ? <p className="section-note">Nenhum arquivo enviado.</p> : <div className="form-list">{files.map((file) => { const id = stringValue(file.id); const ready = file.status === 'READY'; const cancellable = file.status === 'PENDING' || file.status === 'QUARANTINED'; return <article className="form-row" key={id ?? recordTitle(file)}><FileText aria-hidden="true" /><div><strong>{recordTitle(file)}</strong><small>{String(file.status)} · {formatFileSize(file.byteSize)} · ID {id ?? 'indisponível'}</small>{ready && id && <span className="action-row"><a className="secondary-button compact" href={`/api/v1/files/${id}/download`}>Baixar</a></span>}{canManage && cancellable && id && <span className="action-row"><button className="secondary-button compact" type="button" disabled={pending} onClick={() => void cancelUpload(id)}>Cancelar upload</button></span>}</div></article>; })}</div>}</section>
  </>;
}
...
function formatFileSize(value: unknown): string {
  return typeof value === 'number' ? `${(value / 1024).toFixed(1)} KB` : 'tamanho indisponível';
}
...
function EventsModulePage({ canManage }: { canManage: boolean }): React.JSX.Element {
  ...
  const [pending, setPending] = useState(false);
  const load = useCallback(async (): Promise<void> => {
    try { setEvents((await api<{ events: Array<Record<string, unknown>> }>('/v1/events')).events); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível carregar os eventos.'); }
  }, []);
  useEffect(() => { void load(); }, [load]);
  async function create(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api('/v1/events', { method: 'POST', body: JSON.stringify({ code: values.get('code'), title: values.get('title'), origin: values.get('origin'), occurredAt: new Date(String(values.get('occurredAt'))).toISOString(), site: values.get('site') || null, area: values.get('area') || null, description: values.get('description') || null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível registrar o evento.'); }
    finally { setPending(false); }
  }
  async function addAction(eventId: string, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api(`/v1/events/${eventId}/actions`, { method: 'POST', body: JSON.stringify({ title: values.get('title'), owner: values.get('owner') || null, dueAt: values.get('dueAt') ? new Date(String(values.get('dueAt'))).toISOString() : null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível adicionar a ação.'); }
    finally { setPending(false); }
  }
  async function completeAction(eventId: string, actionId: string): Promise<void> {
    setPending(true); setError(null); try { await api(`/v1/events/${eventId}/actions/${actionId}/complete`, { method: 'PATCH' }); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível concluir a ação.'); }
    finally { setPending(false); }
  }
  async function transition(eventId: string, version: number, status: string): Promise<void> {
    setPending(true); setError(null); try { await api(`/v1/events/${eventId}/status`, { method: 'POST', body: JSON.stringify({ expectedVersion: version, status }) }); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível atualizar o estado.'); }
    finally { setPending(false); }
  }
  return <><p className="eyebrow">Módulo empresarial</p><h1 id="dashboard-title">Eventos de segurança</h1><p className="description">Registre ocorrências, ações corretivas e o ciclo de resolução. O sistema impede fechar um evento que tenha ações pendentes.</p>{error && <p className="form-error" role="alert">{error}</p>}{canManage && <form className="inline-form admin-section" onSubmit={create}><label>Código<input name="code" required minLength={2} maxLength={32} placeholder="EV-2026-001" /></label><label>Título<input name="title" required minLength={2} maxLength={200} /></label><label>Origem<input name="origin" required minLength={2} maxLength={80} defaultValue="MANUAL" /></label><label>Ocorrido em<input name="occurredAt" type="datetime-local" required defaultValue={new Date().toISOString().slice(0, 16)} /></label><label>Unidade<input name="site" maxLength={160} /></label><label>Área<input name="area" maxLength={160} /></label><label>Descrição<textarea name="description" maxLength={10000} /></label><button className="primary-button compact" type="submit" disabled={pending}>Registrar evento</button></form>}<section className="admin-section"><h2>Ocorrências da organização</h2>{events.length === 0 ? <p className="section-note">Nenhum evento registrado.</p> : <div className="form-list">{events.map((safetyEvent) => { const eventId = stringValue(safetyEvent.id); const version = typeof safetyEvent.version === 'number' ? safetyEvent.version : 0; const actions = Array.isArray(safetyEvent.actions) ? safetyEvent.actions.filter(isRecord) : []; return <article className="form-row event-detail" key={eventId ?? recordTitle(safetyEvent)}><Settings2 aria-hidden="true" /><div><strong>{stringValue(safetyEvent.code) ?? 'EV'} · {recordTitle(safetyEvent)}</strong><small>{stringValue(safetyEvent.status) ?? 'DRAFT'} · {stringValue(safetyEvent.site) ?? 'Sem unidade'} · {actions.filter((action) => !action.completedAt).length} ação(ões) pendente(s)</small>{actions.length > 0 && <ul className="nested-list">{actions.map((action) => <li key={stringValue(action.id) ?? recordTitle(action)}>{recordTitle(action)} {action.completedAt ? '· concluída' : <>{canManage && eventId && stringValue(action.id) && <button className="secondary-button compact" type="button" disabled={pending} onClick={() => void completeAction(eventId, stringValue(action.id)!)}>Concluir</button>}</>}</li>)}</ul>}{canManage && eventId && <><form className="inline-form compact-form" onSubmit={(formEvent) => void addAction(eventId, formEvent)}><label>Ação corretiva<input name="title" required minLength={2} maxLength={200} /></label><label>Responsável<input name="owner" maxLength={160} /></label><label>Prazo<input name="dueAt" type="datetime-local" /></label><button className="secondary-button compact" type="submit" disabled={pending}>Adicionar ação</button></form><div className="action-row"><select aria-label="Novo estado do evento" defaultValue=""><option value="" disabled>Alterar estado</option>{['DRAFT', 'OPEN', 'IN_REVIEW', 'RESOLVED', 'CLOSED'].map((status) => <option key={status} value={status}>{status}</option>)}</select><button className="secondary-button compact" type="button" disabled={pending || version < 1} onClick={(buttonEvent) => { const status = (buttonEvent.currentTarget.previousElementSibling as HTMLSelectElement | null)?.value; if (status) void transition(eventId, version, status); }}>Atualizar</button></div></>}</div></article>; })}</div>}</section></>;
}

function ChangesModulePage({ canManage, canApprove, identity }: { canManage: boolean; canApprove: boolean; identity: Identity }): React.JSX.Element {
  ...
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState(false);
  const workflowFields: Record<number, Array<{ key: string; label: string }>> = { 1: [{ key: 'scope', label: 'Escopo da mudança' }, { key: 'requester', label: 'Solicitante e partes envolvidas' }], 2: [{ key: 'trigger', label: 'Gatilho da mudança' }, { key: 'impact', label: 'Impacto esperado' }], 3: [{ key: 'implementationPlan', label: 'Plano de implementação' }, { key: 'rollbackPlan', label: 'Plano de retorno' }], 4: [{ key: 'residualRiskAcceptance', label: 'Aceite do risco residual' }], 6: [{ key: 'verificationResult', label: 'Resultado da verificação' }, { key: 'closureDecision', label: 'Decisão de encerramento' }] };
  const load = useCallback(async (): Promise<void> => {
    try { const changeResponse = await api<{ changes: Array<Record<string, unknown>> }>('/v1/changes'); setChanges(changeResponse.changes); if (canManage) { const fileResponse = await api<{ files: Array<Record<string, unknown>> }>('/v1/files'); setReadyFiles(fileResponse.files.filter((file) => file.status === 'READY')); } else setReadyFiles([]); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível carregar as mudanças.'); }
  }, [canManage]);
  useEffect(() => { void load(); }, [load]);
  async function create(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api('/v1/changes', { method: 'POST', body: JSON.stringify({ publicCode: values.get('publicCode'), title: values.get('title'), description: values.get('description') || null, requestedBy: values.get('requestedBy') || null, owner: values.get('owner') || null, dueAt: values.get('dueAt') ? new Date(String(values.get('dueAt'))).toISOString() : null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível criar a mudança.'); }
    finally { setPending(false); }
  }
  async function addRisk(changeId: string, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api(`/v1/changes/${changeId}/risks`, { method: 'POST', body: JSON.stringify({ hazard: values.get('hazard'), consequence: values.get('consequence') || null, probability: Number(values.get('probability')), severity: Number(values.get('severity')), controls: values.get('controls') || null, owner: values.get('owner') || null, dueAt: values.get('dueAt') ? new Date(String(values.get('dueAt'))).toISOString() : null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível adicionar o risco.'); }
    finally { setPending(false); }
  }
  async function addApproval(changeId: string, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api(`/v1/changes/${changeId}/approvals`, { method: 'POST', body: JSON.stringify({ approverName: values.get('approverName'), approverEmail: values.get('approverEmail'), role: values.get('role') || null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível solicitar a aprovação.'); }
    finally { setPending(false); }
  }
  async function decideApproval(changeId: string, approvalId: string, version: number, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api(`/v1/changes/${changeId}/approvals/${approvalId}/decision`, { method: 'POST', body: JSON.stringify({ decision: values.get('decision'), comment: values.get('comment') || null, expectedVersion: version }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível registrar a decisão.'); }
    finally { setPending(false); }
  }
  async function addEvidence(changeId: string, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api(`/v1/changes/${changeId}/evidence`, { method: 'POST', body: JSON.stringify({ fileId: values.get('fileId'), category: values.get('category') || null, description: values.get('description') || null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível vincular a evidência.'); }
    finally { setPending(false); }
  }
  async function completeStep(changeId: string, step: number, version: number, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { const data = Object.fromEntries(Array.from(values.entries()).filter(([key]) => key.startsWith('workflow.')).map(([key, value]) => [key.slice('workflow.'.length), String(value)])); await api(`/v1/changes/${changeId}/steps/${step}/complete`, { method: 'POST', body: JSON.stringify({ notes: values.get('notes'), data, expectedVersion: version }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível concluir a etapa.'); }
    finally { setPending(false); }
  }
  async function transition(changeId: string, version: number, status: string): Promise<void> {
    setPending(true); setError(null); try { await api(`/v1/changes/${changeId}/status`, { method: 'POST', body: JSON.stringify({ expectedVersion: version, status }) }); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível atualizar a etapa.'); }
    finally { setPending(false); }
  }
  return <>
    <p className="eyebrow">Módulo empresarial</p>
    <h1 id="dashboard-title">Gestão de mudanças</h1>
    <p className="description">Registre riscos, defina os aprovadores e conduza a solicitação pelas etapas controladas do processo.</p>
    {error && <p className="form-error" role="alert">{error}</p>}
    {canManage && <form className="inline-form admin-section" onSubmit={create}>
      <label>Código<input name="publicCode" required minLength={2} maxLength={32} placeholder="MUD-2026-001" /></label>
      <label>Título<input name="title" required minLength={2} maxLength={200} /></label>
      <label>Solicitante<input name="requestedBy" maxLength={160} /></label>
      <label>Responsável<input name="owner" maxLength={160} /></label>
      <label>Prazo<input name="dueAt" type="datetime-local" /></label>
      <label>Descrição<textarea name="description" maxLength={10000} /></label>
      <button className="primary-button compact" type="submit" disabled={pending}>Criar mudança</button>
    </form>}
    <section className="admin-section">
      <h2>Solicitações da organização</h2>
      {changes.length === 0 ? <p className="section-note">Nenhuma mudança registrada.</p> : <div className="form-list">{changes.map((change) => {
        const changeId = stringValue(change.id);
        const version = typeof change.version === 'number' ? change.version : 0;
        const status = stringValue(change.status) ?? 'DRAFT';
        const risks = Array.isArray(change.risks) ? change.risks.filter(isRecord) : [];
        const approvals = Array.isArray(change.approvals) ? change.approvals.filter(isRecord) : [];
        const evidence = Array.isArray(change.evidence) ? change.evidence.filter(isRecord) : [];
        const workflowSteps = Array.isArray(change.workflowSteps) ? change.workflowSteps.filter(isRecord) : [];
        const stepLabels = ['Informações gerais', 'Gatilhos e escopo', 'Plano de implementação', 'Avaliação de riscos', 'Aprovação formal', 'Verificação e encerramento'];
        const transitions: Record<string, string[]> = { DRAFT: ['REJECTED'], IN_REVIEW: ['APPROVED', 'REJECTED'], APPROVED: ['IMPLEMENTING'] };
        const allowedStatuses = transitions[status] ?? [];
        return <article className="form-row event-detail" key={changeId ?? recordTitle(change)}>
          <Settings2 aria-hidden="true" />
          <div>
            <strong>{stringValue(change.publicCode) ?? 'MUD'} · {recordTitle(change)}</strong>
            <small>{status} · etapa {String(change.currentStep ?? 1)} · {risks.length} risco(s) · {approvals.length} aprovação(ões)</small>
            <h3 className="subsection-title">Fluxo controlado</h3>
            <ol className="nested-list workflow-steps">{stepLabels.map((label, index) => { const step = index + 1; const entry = workflowSteps.find((item) => stringValue(item.step) === ['GENERAL_INFORMATION', 'TRIGGERS', 'IMPLEMENTATION_PLAN', 'RISK_ASSESSMENT', 'APPROVAL', 'VERIFICATION'][index]); return <li key={label}><strong>{step}. {label}</strong> · {stringValue(entry?.status) ?? (step < Number(change.currentStep ?? 1) ? 'COMPLETED' : 'PENDING')}</li>; })}</ol>
            {risks.length > 0 && <ul className="nested-list">{risks.map((risk) => <li key={stringValue(risk.id) ?? recordTitle(risk)}>{stringValue(risk.hazard) ?? 'Risco'} · P{String(risk.probability)} × S{String(risk.severity)} = {String(risk.score ?? Number(risk.probability) * Number(risk.severity))}</li>)}</ul>}
            <h3 className="subsection-title">Aprovações</h3>
            {approvals.length === 0 ? <p className="section-note">Nenhum aprovador definido.</p> : <ul className="nested-list">{approvals.map((approval) => {
              const approvalId = stringValue(approval.id);
              const decision = stringValue(approval.decision) ?? 'PENDING';
              const approvalVersion = typeof approval.version === 'number' ? approval.version : 0;
              const isDesignatedApprover = stringValue(approval.approverUserId) === identity.user.id;
              return <li key={approvalId ?? recordTitle(approval)}><span>{stringValue(approval.approverName) ?? 'Aprovador'} · {decision}{stringValue(approval.role) ? ` · ${stringValue(approval.role)}` : ''}</span>{canApprove && changeId && approvalId && decision === 'PENDING' && isDesignatedApprover && <form className="inline-form compact-form approval-decision-form" onSubmit={(formEvent) => void decideApproval(changeId, approvalId, approvalVersion, formEvent)}><label>Decisão<select name="decision" defaultValue="APPROVED"><option value="APPROVED">Aprovar</option><option value="REJECTED">Reprovar</option></select></label><label>Comentário<textarea name="comment" maxLength={10000} placeholder="Obrigatório ao reprovar" /></label><button className="secondary-button compact" type="submit" disabled={pending}>Registrar decisão</button></form>}</li>;
            })}</ul>}
            <h3 className="subsection-title">Evidências</h3>
            {evidence.length === 0 ? <p className="section-note">Nenhuma evidência vinculada.</p> : <ul className="nested-list">{evidence.map((item) => { const file = isRecord(item.file) ? item.file : null; const evidenceId = stringValue(item.id); return <li key={evidenceId ?? recordTitle(item)}>{stringValue(file?.originalName) ?? 'Arquivo'}{stringValue(item.category) ? ` · ${stringValue(item.category)}` : ''}{stringValue(item.description) ? ` · ${stringValue(item.description)}` : ''}{changeId && evidenceId && <button className="secondary-button compact" type="button" onClick={() => window.open(`/api/v1/changes/${changeId}/evidence/${evidenceId}/download`, '_blank', 'noopener')}>Baixar</button>}</li>; })}</ul>}
            {canManage && changeId && (status === 'DRAFT' || status === 'IN_REVIEW') && <form className="inline-form compact-form" onSubmit={(formEvent) => void addRisk(changeId, formEvent)}><label>Perigo<input name="hazard" required minLength={2} maxLength={300} /></label><label>Probabilidade<select name="probability" defaultValue="3">{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Severidade<select name="severity" defaultValue="3">{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Controles<textarea name="controls" maxLength={10000} /></label><button className="secondary-button compact" type="submit" disabled={pending}>Adicionar risco</button></form>}
            {canManage && changeId && status === 'IN_REVIEW' && <form className="inline-form compact-form" onSubmit={(formEvent) => void addApproval(changeId, formEvent)}><label>Nome do aprovador<input name="approverName" required minLength={2} maxLength={160} /></label><label>E-mail do aprovador<input name="approverEmail" type="email" required maxLength={320} /></label><label>Papel<input name="role" maxLength={120} placeholder="Ex.: Segurança" /></label><button className="secondary-button compact" type="submit" disabled={pending}>Solicitar aprovação</button></form>}
            {canManage && changeId && <form className="inline-form compact-form" onSubmit={(formEvent) => void addEvidence(changeId, formEvent)}><label>Arquivo pronto<select name="fileId" required disabled={readyFiles.length === 0}><option value="">Selecione</option>{readyFiles.map((file) => <option key={String(file.id)} value={String(file.id)}>{recordTitle(file)}</option>)}</select></label><label>Categoria<input name="category" maxLength={80} placeholder="Ex.: Plano de retorno" /></label><label>Descrição<input name="description" maxLength={10000} /></label><button className="secondary-button compact" type="submit" disabled={pending || readyFiles.length === 0}>Vincular evidência</button></form>}
            {canManage && changeId && ((status === 'DRAFT' && Number(change.currentStep ?? 1) < 5) || (status === 'IMPLEMENTING' && Number(change.currentStep ?? 1) === 6)) && <form className="inline-form compact-form" onSubmit={(formEvent) => void completeStep(changeId, Number(change.currentStep ?? 1), version, formEvent)}>{(workflowFields[Number(change.currentStep ?? 1)] ?? []).map((field) => <label key={field.key}>{field.label}<textarea name={`workflow.${field.key}`} required minLength={2} maxLength={10000} /></label>)}<label>Registro da etapa {String(change.currentStep ?? 1)}<textarea name="notes" required minLength={10} maxLength={10000} placeholder="Descreva a evidência e a decisão desta etapa" /></label><button className="primary-button compact" type="submit" disabled={pending}>Concluir etapa</button></form>}
            {canManage && changeId && allowedStatuses.length > 0 && <div className="action-row"><select aria-label="Nova etapa da mudança" defaultValue=""><option value="" disabled>Alterar etapa</option>{allowedStatuses.map((nextStatus) => <option key={nextStatus} value={nextStatus}>{nextStatus}</option>)}</select><button className="secondary-button compact" type="button" disabled={pending || version < 1} onClick={(buttonEvent) => { const nextStatus = (buttonEvent.currentTarget.previousElementSibling as HTMLSelectElement | null)?.value; if (nextStatus) void transition(changeId, version, nextStatus); }}>Atualizar</button></div>}
          </div>
        </article>;
      })}</div>}
    </section>
...
function BashModulePage({ canManage }: { canManage: boolean }): React.JSX.Element {
  const [cards, setCards] = useState<Array<Record<string, unknown>>>([]);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState(false);
  const stages = ['BACKLOG', 'DESIGN', 'IN_PROGRESS', 'REVIEW', 'DONE'];
  const load = useCallback(async (): Promise<void> => {
    try { setCards((await api<{ cards: Array<Record<string, unknown>> }>('/v1/bash/cards')).cards); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível carregar o quadro BASH.'); }
  }, []);
  useEffect(() => { void load(); }, [load]);
  async function create(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api('/v1/bash/cards', { method: 'POST', body: JSON.stringify({ title: values.get('title'), description: values.get('description') || null, client: values.get('client') || null, criticality: values.get('criticality') || null, assignedTo: values.get('assignedTo') || null, dueAt: values.get('dueAt') ? new Date(String(values.get('dueAt'))).toISOString() : null }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível criar o cartão.'); }
    finally { setPending(false); }
  }
  async function comment(cardId: string, event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null);
    try { await api(`/v1/bash/cards/${cardId}/comments`, { method: 'POST', body: JSON.stringify({ content: values.get('content') }) }); event.currentTarget.reset(); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível comentar no cartão.'); }
    finally { setPending(false); }
  }
  async function move(cardId: string, version: number, stage: string, position: unknown): Promise<void> {
    setPending(true); setError(null); try { await api(`/v1/bash/cards/${cardId}/move`, { method: 'PATCH', body: JSON.stringify({ expectedVersion: version, stage, position: Number(position ?? 0) }) }); await load(); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível mover o cartão.'); }
    finally { setPending(false); }
  }
  return <><p className="eyebrow">Módulo empresarial</p><h1 id="dashboard-title">Quadro BASH</h1><p className="description">Organize a execução por colunas, responsável, criticidade e histórico de comentários.</p>{error && <p className="form-error" role="alert">{error}</p>}{canManage && <form className="inline-form admin-section" onSubmit={create}><label>Título<input name="title" required minLength={2} maxLength={200} /></label><label>Cliente<input name="client" maxLength={160} /></label><label>Criticidade<input name="criticality" maxLength={32} placeholder="Alta" /></label><label>Responsável<input name="assignedTo" maxLength={160} /></label><label>Prazo<input name="dueAt" type="datetime-local" /></label><label>Descrição<textarea name="description" maxLength={10000} /></label><button className="primary-button compact" type="submit" disabled={pending}>Criar cartão</button></form>}<section className="admin-section"><h2>Fluxo de trabalho</h2><div className="board">{stages.map((stage) => <section className="board-column" key={stage}><h3>{stage}</h3>{cards.filter((card) => card.stage === stage).map((card) => { const cardId = stringValue(card.id); const version = typeof card.version === 'number' ? card.version : 0; const comments = Array.isArray(card.comments) ? card.comments.filter(isRecord) : []; return <article className="board-card" key={cardId ?? recordTitle(card)}><strong>{recordTitle(card)}</strong><small>{stringValue(card.client) ?? 'Sem cliente'} · {stringValue(card.criticality) ?? 'Sem criticidade'} · {stringValue(card.assignedTo) ?? 'Sem responsável'}</small>{comments.length > 0 && <ul className="nested-list">{comments.map((item) => <li key={stringValue(item.id) ?? recordTitle(item)}>{stringValue(item.authorName) ?? 'Membro'}: {stringValue(item.content) ?? ''}</li>)}</ul>}{canManage && cardId && <><form className="inline-form compact-form" onSubmit={(formEvent) => void comment(cardId, formEvent)}><label>Comentário<input name="content" required minLength={1} maxLength={10000} /></label><button className="secondary-button compact" type="submit" disabled={pending}>Comentar</button></form><div className="action-row"><select aria-label="Coluna do cartão" defaultValue={stage}>{stages.map((option) => <option value={option} key={option}>{option}</option>)}</select><button className="secondary-button compact" type="button" disabled={pending || version < 1} onClick={(buttonEvent) => { const target = buttonEvent.currentTarget.previousElementSibling as HTMLSelectElement | null; if (target) void move(cardId, version, target.value, card.position); }}>Mover</button></div></>}</article>; })}</section>)}</div></section></>;
}

function HhtModulePage({ canManage }: { canManage: boolean }): React.JSX.Element {
  ...
  const [pending, setPending] = useState(false);
  const load = useCallback(async (): Promise<void> => {
    try { const data = await api<{ companies: Array<Record<string, unknown>>; reports: Array<Record<string, unknown>>; windows: Array<Record<string, unknown>> }>('/v1/hht'); setCompanies(data.companies); setReports(data.reports); setWindows(data.windows); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível carregar o HHT.'); }
  }, []);
  useEffect(() => { void load(); }, [load]);
  async function createCompany(event: FormEvent<HTMLFormElement>): Promise<void> { event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null); try { await api('/v1/hht/companies', { method: 'POST', body: JSON.stringify({ name: values.get('name'), document: values.get('document') || null, site: values.get('site'), coordination: values.get('coordination') || null }) }); event.currentTarget.reset(); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível cadastrar a empresa.'); } finally { setPending(false); } }
  async function saveWindow(event: FormEvent<HTMLFormElement>): Promise<void> { event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(null); try { await api('/v1/hht/windows', { method: 'PUT', body: JSON.stringify({ year: Number(values.get('year')), month: Number(values.get('month')), opensAt: new Date(String(values.get('opensAt'))).toISOString(), closesAt: new Date(String(values.get('closesAt'))).toISOString() }) }); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível definir a janela.'); } finally { setPending(false); } }
  async function saveReport(event: FormEvent<HTMLFormElement>): Promise<void> { event.preventDefault(); const values = new FormData(event.currentTarget); const companyId = String(values.get('companyId') ?? ''); const year = Number(values.get('year')); const month = Number(values.get('month')); const existing = reports.find((report) => report.companyId === companyId && report.year === year && report.month === month); const existingVersion = typeof existing?.version === 'number' ? existing.version : undefined; setPending(true); setError(null); try { await api('/v1/hht/reports', { method: 'PUT', body: JSON.stringify({ companyId, year, month, hhtWorked: Number(values.get('hhtWorked')), hhtMeal: Number(values.get('hhtMeal')), workforce: Number(values.get('workforce')), lostDays: Number(values.get('lostDays')), lti: Number(values.get('lti')), expectedVersion: existingVersion }) }); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível salvar o relatório.'); } finally { setPending(false); } }
  async function setReportStatus(reportId: string, status: string, expectedVersion: number): Promise<void> { setPending(true); setError(null); try { await api(`/v1/hht/reports/${reportId}/status`, { method: 'POST', body: JSON.stringify({ status, expectedVersion }) }); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Não foi possível alterar o status do relatório.'); } finally { setPending(false); } }
  const today = new Date();
  return <><p className="eyebrow">Módulo empresarial</p><h1 id="dashboard-title">HHT e taxas</h1><p className="description">Cadastre unidades, abra a janela de reporte e acompanhe os indicadores normalizados por período.</p>{error && <p className="form-error" role="alert">{error}</p>}{canManage && <><form className="inline-form admin-section" onSubmit={createCompany}><label>Empresa/unidade<input name="name" required minLength={2} maxLength={200} /></label><label>Local<input name="site" required minLength={2} maxLength={120} /></label><label>Documento<input name="document" maxLength={32} /></label><label>Coordenação<input name="coordination" maxLength={160} /></label><button className="primary-button compact" type="submit" disabled={pending}>Cadastrar unidade</button></form><form className="inline-form admin-section" onSubmit={saveWindow}><label>Ano<input name="year" type="number" defaultValue={today.getFullYear()} min="2000" max="2200" required /></label><label>Mês<input name="month" type="number" defaultValue={today.getMonth() + 1} min="1" max="12" required /></label><label>Abre em<input name="opensAt" type="datetime-local" required defaultValue={today.toISOString().slice(0, 16)} /></label><label>Fecha em<input name="closesAt" type="datetime-local" required defaultValue={new Date(today.getTime() + 7 * 86400000).toISOString().slice(0, 16)} /></label><button className="secondary-button compact" type="submit" disabled={pending}>Salvar janela</button></form><form className="inline-form admin-section" onSubmit={saveReport}><label>Unidade<select name="companyId" required disabled={companies.length === 0}><option value="">Selecione</option>{companies.map((company) => <option key={String(company.id)} value={String(company.id)}>{recordTitle(company)}</option>)}</select></label><label>Ano<input name="year" type="number" defaultValue={today.getFullYear()} min="2000" max="2200" required /></label><label>Mês<input name="month" type="number" defaultValue={today.getMonth() + 1} min="1" max="12" required /></label><label>HHT trabalhadas<input name="hhtWorked" type="number" min="0" step="0.01" required /></label><label>HHT refeição<input name="hhtMeal" type="number" min="0" step="0.01" required /></label><label>Efetivo<input name="workforce" type="number" min="0" required /></label><label>Dias perdidos<input name="lostDays" type="number" min="0" required /></label><label>Acidentes LTI<input name="lti" type="number" min="0" required /></label><button className="primary-button compact" type="submit" disabled={pending || companies.length === 0}>Salvar reporte</button></form></>}<section className="admin-section"><h2>Janelas e relatórios</h2>{windows.length > 0 && <p className="section-note">Janelas: {windows.map((window) => `${String(window.month).padStart(2, '0')}/${String(window.year)}`).join(', ')}</p>}{reports.length === 0 ? <p className="section-note">Nenhum relatório registrado.</p> : <div className="form-list">{reports.map((report) => { const rate = isRecord(report.rates) ? report.rates : {}; const reportId = stringValue(report.id); const version = typeof report.version === 'number' ? report.version : 0; return <article className="form-row" key={reportId ?? recordTitle(report)}><Settings2 aria-hidden="true" /><div><strong>{isRecord(report.company) ? recordTitle(report.company) : 'Unidade'} · {String(report.month).padStart(2, '0')}/{String(report.year)}</strong><small>{String(report.status)} · TRIFR {String(rate.trifr ?? 0)} · LTISR {String(rate.ltisr ?? 0)}</small></div>{canManage && reportId && <span className="field-actions">{report.status === 'DRAFT' && <button className="secondary-button compact" type="button" disabled={pending || version < 1} onClick={() => void setReportStatus(reportId, 'SUBMITTED', version)}>Enviar</button>}{report.status === 'SUBMITTED' && <button className="secondary-button compact" type="button" disabled={pending || version < 1} onClick={() => void setReportStatus(reportId, 'LOCKED', version)}>Bloquear</button>}</span>}</article>; })}</div>}</section></>;
}
