...
type DashboardView = 'home' | 'forms' | 'notifications' | 'events' | 'changes' | 'bash' | 'hht' | 'dashboards' | 'tv' | 'integrations' | 'operations' | 'classifications' | 'files' | 'organization' | 'profile';
...
const menuGroups: Array<{ label: string; items: Array<{ view: DashboardView; label: string; icon: typeof LayoutDashboard }> }> = [
  ...
  { label: 'Administração', items: [{ view: 'operations', label: 'Operação', icon: ShieldCheck }, { view: 'integrations', label: 'Integrações', icon: Settings2 }, { view: 'classifications', label: 'Listas', icon: ClipboardList }, { view: 'files', label: 'Arquivos', icon: FileText }, { view: 'organization', label: 'Organização', icon: UsersRound }] }
];
...
export function DashboardShell({ children, collapsed, identity, onLogout, onNavigate, onToggle, pending, unreadNotifications, view }: DashboardShellProps): React.JSX.Element {
  ...
  return <main className={`dashboard-shell${collapsed ? ' sidebar-collapsed' : ''}`}>
    <aside className="dashboard-sidebar" aria-label="Navegação principal">
      ...
      <nav className="sidebar-nav">{menuGroups.map((group) => <section className="sidebar-group" key={group.label} aria-label={group.label}><p>{group.label}</p>{group.items.filter((item) => item.view !== 'operations' || identity.membership.role === 'OWNER' || identity.membership.role === 'ADMIN').map((item) => { const Icon = item.icon; return <button className="sidebar-item" data-active={view === item.view || undefined} key={item.view} type="button" aria-current={view === item.view ? 'page' : undefined} title={collapsed ? item.label : undefined} onClick={() => onNavigate(item.view)}><Icon aria-hidden="true" /><span>{item.label}</span></button>; })}</section>)}</nav>
