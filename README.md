...
4. Crie domínio HTTPS para `web` (porta interna `80`). A API fica interna; exponha-a apenas se houver integração externa, com domínio próprio. Em `APP_ORIGIN`, informe todas as origens públicas exatas, separadas por vírgula (por exemplo, domínio raiz e `www`).
