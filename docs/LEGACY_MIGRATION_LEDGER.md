...
| CFG-01 | `/settings`, `/dropdown-lists`; `SystemSetting`, `DropdownList`; `SettingsPage`, `CustomizationPage` | configurações, listas e aparência | migrar seletivamente | `TenantSetting` tipado e catálogo de opções | F1 / R1 | TBD / TBD | chaves aprovadas e JSON validado; nenhuma credencial no payload |
| FOR-01 | `/forms`; `Form`, `FormSlugHistory`, `FormField`, `FormPermission`, `FormFieldCounter`; `FormManagerPage`, `FormConfigPage`, `FormFieldsPage`, componentes `forms/*` | builder, campos, regras e permissões | substituir | Form, FormVersion, FieldDefinition, ResourceGrant | F2 / R2 | TBD / TBD | formulários/versões/campos por tenant; snapshot canônico de configuração |
...
| BSH-01 | `/bash-cards`; `BashCard`, `BashCardComment`, `BashCardAttachment`, `BashBoardSetting`; `GestaoBashPage`, `components/bash/*` | quadro BASH, comentários e anexos | substituir | BashCard, Comment, FileAsset e workflow | F3 / R3 | TBD / TBD | cartões por estágio, comentários e checksums de anexos |
...
| TV-01 | `/tv-displays`, `/tv-playlists`; `FormTvDisplay`, `TvPlaylist`, `TvPlaylistItem`; `TvDisplay*`, `TvPlaylist*` | displays, playlists e TV pública | substituir | Display público revogável já entregue; Playlist pública no próximo incremento | F4 / R4 | Display entregue / Playlist TBD | Display usa token opaco, snapshot estático, expiração, RLS, auditoria e outbox; Playlist precisa revalidar cada Display em toda rotação |
| OPS-01 | `/monitoring`; `ServerMetricsCard`, `ServersPage`, `useServerMonitoring` | estado e monitoramento operacional | substituir | métricas/alertas do runtime, sem métricas de host por usuário | F4 / R4 | TBD / TBD | SLI/SLO, alerta e runbook aprovados |
