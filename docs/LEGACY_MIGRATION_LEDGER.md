...
| ANL-01 | `/dashboard`, `/dashboards`, `/dashboard-data`; `Dashboard`, `DashboardView`; `DashboardBuilderPage`, widgets `dashboards/*`, `PublicDashboardPage` | dashboards, widgets e publicação | migrar seletivamente | Dashboard, WidgetDefinition, DataSource e Publication | F4 / R4 | TBD / TBD | layouts/widgets aprovados; resultados de consultas em amostra por tenant |
| TV-01 | `/tv-displays`, `/tv-playlists`; `FormTvDisplay`, `TvPlaylist`, `TvPlaylistItem`; `TvDisplay*`, `TvPlaylist*` | displays, playlists e TV pública | substituir | Display, Playlist e Publication revogável | F4 / R4 | TBD / TBD | displays/playlists ativos e links públicos revogáveis |
