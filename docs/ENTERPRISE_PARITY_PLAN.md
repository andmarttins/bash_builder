...
| P6 — painéis, relatórios e TV | `/app/paineis`, editor, `/app/relatorios`, `/app/tv`; público `/p/:publicId`, `/tv/:publicId`, `/tv/playlist/:publicId` | `Dashboard*`, `FormTvDisplay`, `TvPlaylist*`, `BiApiKey` | consultas parametrizadas, blocos de painel, playlist e permissões públicas | snapshots/agregados e invalidação de cache |
