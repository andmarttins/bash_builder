...
| --- | --- | --- | --- | --- |
| F1 | **Escopo e dados do piloto.** Consolidar carta do piloto, inventário legado, de-para de tenant/identidade/ownership e exceções. | `BLOQUEADO` | Carta aprovada; todas as linhas do piloto decididas; manifesto criptografado com hashes; carga de identidade reconciliada. | Aplicar a decisão de escopo em `docs/migration/PILOT_DECISION_2026-09-22.md` aos dados reais e registrar as evidências P0.1–P0.3. |
| F2 | **Decisões funcionais e autorização.** Fechar grants, configurações/catálogos, deltas de formulários, BASH, HHT e reconciliação dos domínios aprovados. | `BLOQUEADO` | ADRs e aceites aprovados; somente decisões aprovadas implementadas, auditadas, isoladas por tenant e reconciliadas. | Executar o baseline decidido: RBAC atual, sem grants/deltas novos; registrar aceite e reconciliação para o piloto. |
| F3 | **Integrações e operação.** Homologar webhook/adapters exigidos e observabilidade de produção, com segredos protegidos e operação exercitada. | `BLOQUEADO` | Integrações do piloto homologadas; retry/DLQ/rotação/re-drive testados; SLOs, alertas, retenção e plantão aprovados. | Homologar apenas WEBHOOK e registrar destino, responsável, alertas, retenção e plantão. |
...
| 22/09/2026 | F1–F5, decisão de escopo | baseline conservador decidido: piloto de uma organização representativa, RBAC atual sem grants, sem adapters adicionais, webhook único e corte reversível. A decisão não substitui dados, aprovações ou evidências reais. | `docs/migration/PILOT_DECISION_2026-09-22.md` |
