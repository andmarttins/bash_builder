DROP POLICY "change_requests_tenant_isolation" ON "change_requests";
...
BEGIN
  ...
  WITH candidates AS (
    SELECT cr.organization_id, cr.id AS change_id, cr.due_at,
      CASE WHEN cr.due_at < p_now THEN 'change.deadline_escalated' ELSE 'change.deadline_reminder' END AS notification_type,
  ...
  WHERE NOT EXISTS (
    ...
    WHERE projection.event_id = identified.notification_id AND projection.projection_name = 'change-deadline-monitor-v1'
  )
